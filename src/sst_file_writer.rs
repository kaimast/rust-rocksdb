...
pub struct SstFileWriter<'a> {
    pub(crate) inner: *mut ffi::rocksdb_silk_sstfilewriter_t,
...
struct EnvOptions {
    inner: *mut ffi::rocksdb_silk_envoptions_t,
}

impl Drop for EnvOptions {
    fn drop(&mut self) {
        unsafe {
            ffi::rocksdb_silk_envoptions_destroy(self.inner);
        }
    }
}
...
impl Default for EnvOptions {
    fn default() -> EnvOptions {
        let opts = unsafe { ffi::rocksdb_silk_envoptions_create() };
...
impl<'a> SstFileWriter<'a> {
    ...
    fn create_raw(opts: &Options, env_opts: &EnvOptions) -> *mut ffi::rocksdb_silk_sstfilewriter_t {
        unsafe { ffi::rocksdb_silk_sstfilewriter_create(env_opts.inner, opts.inner) }
    }
    ...
    fn open_raw(&'a self, cpath: &CString) -> Result<(), Error> {
        unsafe {
            ffi_try!(ffi::rocksdb_silk_sstfilewriter_open(
                self.inner,
    ...
    pub fn finish(&mut self) -> Result<(), Error> {
        unsafe {
            ffi_try!(ffi::rocksdb_silk_sstfilewriter_finish(self.inner,));
            Ok(())
        }
    }
/*
    ...
    pub fn file_size(&self) -> u64 {
        ...
        unsafe { ffi::rocksdb_silk_sstfilewriter_file_size(self.inner, &mut file_size) };
        file_size
    }*/
        ...
        V: AsRef<[u8]>,
    {
        ...
        unsafe {
            ffi_try!(ffi::rocksdb_silk_sstfilewriter_put(
                self.inner,
        ...
        V: AsRef<[u8]>,
    {
        ...
        unsafe {
            ffi_try!(ffi::rocksdb_silk_sstfilewriter_merge(
                self.inner,
                key.as_ptr() as *const c_char,
                key.len() as size_t,
                value.as_ptr() as *const c_char,
    ...
    pub fn delete<K: AsRef<[u8]>>(&mut self, key: K) -> Result<(), Error> {
        ...
        unsafe {
            ffi_try!(ffi::rocksdb_silk_sstfilewriter_delete(
                self.inner,
                key.as_ptr() as *const c_char,
                key.len() as size_t,
            ));
...
impl<'a> Drop for SstFileWriter<'a> {
    fn drop(&mut self) {
        unsafe {
            ffi::rocksdb_silk_sstfilewriter_destroy(self.inner);
        }
    }
}
