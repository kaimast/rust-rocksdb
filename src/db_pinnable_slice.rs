...
pub struct DBPinnableSlice<'a> {
    ptr: *mut ffi::rocksdb_silk_pinnableslice_t,
    db: PhantomData<&'a DB>,
}
...
impl<'a> Deref for DBPinnableSlice<'a> {
    ...
    fn deref(&self) -> &[u8] {
        unsafe {
            ...
            let val = ffi::rocksdb_silk_pinnableslice_value(self.ptr, &mut val_len) as *mut u8;
...
impl<'a> Drop for DBPinnableSlice<'a> {
    fn drop(&mut self) {
        unsafe {
            ffi::rocksdb_silk_pinnableslice_destroy(self.ptr);
        }
    }
}
...
impl<'a> DBPinnableSlice<'a> {
    ...
    ///
    /// # Unsafe
    /// Requires that the pointer must be generated by rocksdb_silk_get_pinned
    pub(crate) unsafe fn from_c(ptr: *mut ffi::rocksdb_silk_pinnableslice_t) -> DBPinnableSlice<'a> {
        DBPinnableSlice {
            ptr,
            db: PhantomData,
        }
